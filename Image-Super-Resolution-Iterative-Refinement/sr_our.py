...
def visualize(visuals,t,result_path,current_step,idx):
    ...
    sr_img = Metrics.s2_to_img(visuals['SR'][t_step-1,:,:,:], percentile=(0.5, 99.5), gamma=0.9, bands = [2,1,0],min_max=(-1, 1))
    hr_img = Metrics.s2_to_img(visuals['HR'][0,:,:,:], percentile=(0.5, 99.5), gamma=0.9, bands = [2,1,0],min_max=(-1, 1))
    lr_img = Metrics.s2_to_img(visuals['LR'][0,:,:,:], percentile=(0.5, 99.5), gamma=0.9, bands = [2,1,0],min_max=(-1, 1))
    fake_img = Metrics.s2_to_img(visuals['INF'][0,:,:,:], percentile=(0.5, 99.5), gamma=0.9, bands = [2,1,0],min_max=(-1, 1))
    # generation
    ...
    if opt['phase'] == 'train':
        while current_step < n_iter:
            ...
            for batch_num, train_data in enumerate(train_loader):
                ...
                # print(opt)
                diffusion.optimize_parameters(batch_num)
                # log
